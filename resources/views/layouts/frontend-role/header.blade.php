...
<body>
    ...
    use App\Models\User;
    $upgradeBtn =0;
    $log_role_id=Auth::guard('roleuser')->user()->user_role_id;
    $parent = User::where('id',Auth::guard('roleuser')->user()->parent_id)->first();
    $currentDate = date('Y-m-d');
    if($parent->plan_id != 0 && $parent->parent_id == 0 && $parent->user_role_id == 1){//parent - own team user
        $renewal_list = Renewal_details::where('user_id',$parent->id)->where('is_activate','1')->where('status',1)->latest()->first();
        if($renewal_list){
    ...
    }else if($parent->plan_id == 0 && $parent->parent_id != 0 && $parent->user_role_id == 1){//parent - team user of enterpriser
        $getEnterpriser = User::where('id',$parent->parent_id)->where('user_role_id',4)->first();
        if($getEnterpriser){
            $renewal_list = Renewal_details::where('user_id',$getEnterpriser->id)->where('is_activate','1')->where('status',1)->latest()->first();
            if($renewal_list){
                if(date('Y-m-d',strtotime($renewal_list['renewal_date'])) <= $currentDate){
                    $upgradeBtn =1;
                }
    ...
    <div class="container-fluid default_theme">
        <div class="row">
            ...
            <input type="hidden" id="log_role_id" value="{{$log_role_id}}">
            <div class="col-12 d-flex p-0" id="wrapper" style="min-height: 100vh;">
                @include('layouts/frontend-role/sidebar')
                <div class="col p-0">
                        ...
                        <nav class="navbar navbar-expand-md fixed-top py-2 px-lg-4 webview">
                            <div class="container-fluid px-1">
                                <div class="d-flex align-items-center">
                                    ...
                                    <h2 class="m-0 welcome_text">Welcome,  {{ucfirst(Auth::guard('roleuser')->user()->name)}}  ({{ optional(Auth::guard('roleuser')->user()->userTeamRole)->role  }})</h2>
                                </div>
                                ...
                                <div class="collapse navbar-collapse mt-lg-0 mt-3 p-0 mr-3" id="navbarSupportedContent1">
                                    ...
                                    <div class="XA_text mr-4 mb-lg-0 mb-3" id="XA">
                                        <a>{{strtoupper(Auth::guard('roleuser')->user()->name[0].Auth::guard('roleuser')->user()->name[1])}}</a>
                                        <div id="logout_btn"> 

                                           <a href="{{route('role-user.account-setting')}}">
...
<script>
    $(document).ready(function(){
        ...
        if(upgrade == 1){
           // alert('Your admin renewal has been expired,  please contact your admin');
           window.location.href = baseurl+'/role-user-logout/1';                          
        }else if(upgrade == 2 || upgrade == 3){
            // alert('Your admin is not a valid user!');
            window.location.href = baseurl+'/role-user-logout/2'; 
        }   
        $(".flow").on({ 
          click: function(){ 
    ...
    function removeNotification(){
        ...
        $.ajax({
            url:  '{{ route("role-update-notification") }}', 
            type: 'POST',
    ...
    function getNotification(){
      var baseurl = $('#base_url').val();  
      var log_role_id = $('#log_role_id').val();
      let URL =  '{{ route("role-get-notification") }}'; 
      var notificationData='';
      $.get(URL,
          function(response) {
              if(response['cnt'] != 0 ){ 
                $('.badge').html(response['cnt']);
                 notificationData+='<div class="pt-2 notify_content">';
                if(response['usertemplate'].length > 0){ 
                    $.each(response['usertemplate'], function (key, value) {
                        var furl = baseurl+'/RU-flowchart?user='+value['user_template_id']+'&process=received';
                 ...
                 if(response['notifications'].length > 0){ 
                    $.each(response['notifications'], function (key, value2) {
                        if(value2['type'] != 4){
                            if(value2['type'] == 6){//approved
                                if(log_role_id != 4 ){
                                    var furl = baseurl+'/RU-flowchart?user='+value2['template_id']+'&notif-'+value2['id'];
                                }else if(log_role_id == 4){
                                    var furl = baseurl+'/RU-flowchart?user='+value2['template_id']+'&process=qrcode';
                                }
                            }else{
                                var furl = baseurl+'/RU-flowchart?user='+value2['template_id']+'&notif-'+value2['id'];
                            }
                        }else{
                            var furl=baseurl+'/role-user/view-projects/'+value2['template_id'];
                        }
                        notificationData+='<div class="notify_part d-flex"><div class="notify_img"><img src="{{asset('images/front_images/user_module/user.png')}}" alt=""></div><div class="notify_para"><h3><a href="'+furl+'">'+value2['message']+'</a></div></div>';
                    });
                }
...
<script>
   ...
   function getUserTheme(){
          let URL =  '{{ route("role-user.set-theme") }}';
          $.get(URL,
          function(response) {
