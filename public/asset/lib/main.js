...
function updateShape(shapeId, property, newValue, skipCommand, previousValue){
   // console.log(shapeId+'------'+property+'--------'+newValue+'---------'+skipCommand+'-------'+previousValue);
...
function zoomin(zoomIn) {
    var GFG = document.getElementById("a");
...
function zoom(zoomOut) {
    var GFG = document.getElementById("a");
    var currWidth = GFG.clientWidth;
    GFG.style.width = "900px";
}

function deleteshape(delshpae) {   
    
    // alert(delshpae); 
 var selectedFigureId=delshpae;
    if(selectedFigureId != -1){  
       var cmdDelFig = new FigureDeleteCommand(selectedFigureId);
       cmdDelFig.execute();
       History.addUndo(cmdDelFig);   
...
function setShadow(shadow) {   
    // alert(shadow); 
    if(shadow != -1){ 
        var SHADOWSHAPE = STACK.figureGetById(shadow);
        var bounds = SHADOWSHAPE.getBounds(); 
        
        // alert(SHADOWSHAPE);
        SHADOWSHAPE.style.filter ="drop-shadow(3px 8px 3px gray)";    
        // body.appendChild(SHADOWSHAPE);
        draw();
        return false; 
    }
    ...
    switch(action){
       ...
       case 'redo':
        ...
        case 'zoomin':
            ...
            break;
        case 'deleteshape':
            deleteshape(selectedFigureId);
            break;
        case 'setShadow':
            setShadow(selectedFigureId);
            break;
