...
use App\Http\Requests\ManageTrialUserRequest;
use App\Models\User;
...
class ManageUserController extends Controller
{
    ...
    public function createTrialUser(Request $request, $id = null){
        $userAdd['module'] = 'trial-users';
        ...
        //dd($userDetail);
        return view('super-admin.trial-user.create',['user'=>$user,'userDetail'=>$userDetail,'Module'=>$userAdd,'subscriptions'=>$subscriptions]);
    }
    public function storeTrialUser(ManageTrialUserRequest $request,$id = null){
        ...
        if($chkMobile && $chkuser == 1){ 
            ...
            if($chk){
                if($chk['parent_id'] != 0){
                    return redirect('create/trial/'.$id)->withInput()->withErrors(['contact_no' => trans('The Contact No has already been taken.')]);
                }
        ...
        $planDetail = Subscription::find($request->subscription_plan);
        if($id){
        ...
        $userInsert->user_role_id = $planDetail->user_role_id;
        $userInsert->plan_id = $request->subscription_plan;
        if($id){
        ...
        if(!$userRenewal){
            ...
            if($request->change_plan == '1'){
                $currentDate = date('Y-m-d');
                $fcUpdate=$this->subUserUpgrade($user_id,$planDetail->user_role_id,$db_user_role);
            }else{
        ...
        if(!$userRenewal){
            ...
            //if($db_user_role != $planDetail->user_role_id){
            if($request->change_plan == '1'){
        ...
        if($planDetail->user_role_id=="1"){
           if($id){                
               ...
               if($id){                
                    return redirect('trial-users')->with('status','Trail User Updated Successfully');
                }
                else{                
                    return redirect('trial-users')->with('status','Trail User Added Successfully');            
                }
    ...
    public function getTrialUser(Request $request){
        $userlist['module'] = 'trial-users';
         ...
         $users = User::with(['userRenewalDatail'=> function ($query) {
            $query->where('is_activate','1')->where('status','1');
                ...
                if(request()->mobile != ''){
                    ...
                    $users->whereHas('userDetail' , function($query) use ($mobile,$request) {
                      $query->where('contact_no','like', '%' .$mobile. '%');
                    });
                }
                $users->whereHas('userRenewalDatail' , function($query) use ($request) {
                  $query->where('is_activate','1')->where('status','1');
                }); 

        $users = $users->where('user_role_id',2)->orderBy('created_at', 'desc')->paginate(20);
         $plans =  User::join('user_details','users.id','user_details.user_id')                   
                    ...
                    ->join('super_admin_subscription_plan','renewal_details.plan_id','super_admin_subscription_plan.id') 
                     ->whereIn('renewal_details.is_activate',['0','1'])->where('users.user_role_id',2)->where('renewal_details.status',1)                  
        ...
        return view('super-admin.trial-user.list',compact('users','plans'),['Module'=>$userlist])->with('user_name', $user_name)->with('email', $email)->with('mobile', $mobile)->with('status', $status);
    }
    
    public function deleteTrialUser($id){ 
            $proIds=[]; 
            $temp_id=[];
            $getNotify=[];
            $getTemplates=[];
        ...
        if($id){
            ...
            return redirect('trial-users')->with('status','Trail User Deleted Successfully');
        }
    }
