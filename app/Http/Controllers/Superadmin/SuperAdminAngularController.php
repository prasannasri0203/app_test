...
class SuperAdminAngularController extends Controller
{
    ...
    public function saveFlowchartTest(){
    	$flowcharts=UserTemplate::where('user_id',Auth::guard('superadmin')->user()->id)->get();
    	$projects=FlowchartProject::where('created_by',Auth::guard('superadmin')->user()->id)->get();
...
if (file_exists($jsonfile_path)) {
    unlink($jsonfile_path);
			       ...
			       $name = 'USER_' . Auth::guard('superadmin')->user()->id . '_FLW_' . rand(11111, 99999) . date('_d_m_Y');
...
/*default image save*/
            ...
            $flowchart_save->status=$request->status;
					...
					$flowchart_save->updated_by=Auth::guard('superadmin')->user()->id;
					$flowchart_save->save();
					...
					$flowchart_track->user_id=Auth::guard('superadmin')->user()->id;
					if($request->status){
    ...
    public function flowchartsWithCommentsSave(Request $request){
         try {
				...
				$comment_save->user_id=Auth::guard('superadmin')->user()->id;
				$comment_save->user_name=Auth::guard('superadmin')->user()->name;
    ...
    public function flowchartsWithCommentsList($id){
         try {
		        ...
		        $Authuser=Auth::guard('superadmin')->user()->id;
    ...
    public function flowchartsWithNotesSave(Request $request,$id = null){
    	try {
				...
				$note_save->user_id=Auth::guard('superadmin')->user()->id;
       ...
       $temp_detail=UserTemplate::find($temp_id);
        ...
        if(($user_detail->parent_id==0) && ($user_detail->user_role_id == 1 || $user_detail->user_role_id== 4)){
            ...
            $ids = array_filter($ids, function($k) {
              return $k != Auth::guard('superadmin')->user()->id;
            });
            if(count($ids) > 0){
