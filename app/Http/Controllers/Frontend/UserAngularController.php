...
class UserAngularController extends Controller
{
...
if (file_exists($jsonfile_path)) {
    unlink($jsonfile_path);
			       ...
			       $name = 'USER_' . Auth::user()->id . '_FLW_' . rand(11111, 99999) . date('_d_m_Y');
...
/*default image save*/
    ...
    public function sharedUpdateNotification($id){
        $notifications = Notification::where('to_id',Auth::user()->id)->where('status',0)->get();
